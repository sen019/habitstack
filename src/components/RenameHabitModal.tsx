...
function ModalContent({ habit, onClose, onRename, habitName, setHabitName }: {
  habit: Habit;
  onClose: () => void;
  onRename: (habitId: string, name: string) => void;
  habitName: string;
  ...
  const handleSubmit = () => {
    if (habitName.trim() && habitName.trim() !== habit.name) {
      onRename(habit.id, habitName.trim());
      setHabitName('');
    } else {
      onClose();
    }
  ...
  return (
    <div className="bg-background box-border content-stretch flex flex-col gap-[24px] items-start p-[24px] relative rounded-tl-[var(--radius)] rounded-tr-[var(--radius)] shrink-0 w-full" data-name="modal">
      ...
      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full" data-name="card-header">
        <p className="font-['Geist',_sans-serif] font-semibold leading-[normal] not-italic relative shrink-0 text-[18px] text-foreground text-nowrap whitespace-pre">Rename habit</p>
        <button
          onClick={onClose}
        ...
        >
          <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
            <div className="box-border content-stretch flex gap-[8px] h-[58px] items-center justify-center px-[24px] py-[12px] relative w-full">
              <p className="font-['Geist',_sans-serif] font-semibold leading-[normal] not-italic relative shrink-0 text-[18px] text-primary-foreground text-nowrap whitespace-pre">Save Changes</p>
            </div>
          </div>
        </button>
      </div>
...
export default function RenameHabitModal({ isOpen, habit, onClose, onRename }: RenameHabitModalProps) {
